...
static inline DirectX::XMVECTOR ReplaceXMVectorPermute(DirectX::XMVECTOR v1, DirectX::XMVECTOR v2, DirectX::XMVECTORI32 p )
{
    
    return XMVectorPermute(v1,v2, p.i[0], p.i[1], p.i[2], p.i[3]);
}
//-----------------------------------------------------------------------------
...
static inline BOOL XMVector3AnyTrue(DirectX::FXMVECTOR V )
{
    ...
    return DirectX::XMComparisonAnyTrue(DirectX::XMVector4EqualIntR( C, DirectX::XMVectorTrueInt() ) );
}
...
static inline BOOL XMVector3AllTrue(DirectX::FXMVECTOR V )
{
    ...
    return DirectX::XMComparisonAllTrue(DirectX::XMVector4EqualIntR( C, DirectX::XMVectorTrueInt() ) );
}
...
// (V0, V1, V2).
//-----------------------------------------------------------------------------
static inline DirectX::XMVECTOR PointOnPlaneInsideTriangle(DirectX::FXMVECTOR P, DirectX::FXMVECTOR V0, DirectX::FXMVECTOR V1, DirectX::FXMVECTOR V2 )
{
...
VOID ComputeBoundingSphereFromPoints( Sphere* pOut, UINT Count, const DirectX::XMFLOAT3* pPoints, UINT Stride )
{
    ...
    MinX = MaxX = MinY = MaxY = MinZ = MaxZ = DirectX::XMLoadFloat3( pPoints );

    for( UINT i = 1; i < Count; i++ )
    {
        DirectX::XMVECTOR Point = DirectX::XMLoadFloat3( (DirectX::XMFLOAT3* )( ( BYTE* )pPoints + i * Stride ) );
    ...
    DirectX::XMStoreFloat3( &pOut->Center, Center );
    DirectX::XMStoreFloat( &pOut->Radius, Radius );
...
VOID ComputeBoundingAxisAlignedBoxFromPoints( AxisAlignedBox* pOut, UINT Count, const DirectX::XMFLOAT3* pPoints, UINT Stride )
{
  /*  ////XMASSERT( pOut );
    ...
    vMin = vMax = DirectX::XMLoadFloat3( pPoints );
    ...
    DirectX::XMStoreFloat3( &pOut->Center, ( vMin + vMax ) * 0.5f );
    DirectX::XMStoreFloat3( &pOut->Extents, ( vMax - vMin ) * 0.5f );
                                             ...
                                             FLOAT m22, FLOAT m23, FLOAT m33, FLOAT e )
{
    ...
    DirectX::XMVECTOR vTmp = DirectX::XMLoadFloat3( (XMFLOAT3*)fTmp );
...
VOID ComputeBoundingOrientedBoxFromPoints( OrientedBox* pOut, UINT Count, const DirectX::XMFLOAT3* pPoints, UINT Stride )
{
    ...
    static CONST DirectX::XMVECTORI32 PermuteYZZ =
                 {
    ...
    for( UINT i = 0; i < Count; i++ )
    {
        DirectX::XMVECTOR Point = DirectX::XMLoadFloat3( ( DirectX::XMFLOAT3* )( ( BYTE* )pPoints + i * Stride ) ) - CenterOfMass;

        XX_YY_ZZ += Point * Point;

        DirectX::XMVECTOR XXY = ReplaceXMVectorPermute(Point, Point, PermuteXXY);
        DirectX::XMVECTOR YZZ = ReplaceXMVectorPermute( Point, Point, PermuteYZZ );
    ...
    // coordinate system (Det ~= -1) because DirectX::XMQuaternionRotationMatrix only 
    ...
    {
        const DirectX::XMVECTORF32 VectorNegativeOne =
        {
            -1.0f, -1.0f, -1.0f, -1.0f
        };
    ...
    DirectX::XMVECTOR Orientation = DirectX::XMQuaternionRotationMatrix( R );
    ...
    vMin = vMax = DirectX::XMVector3TransformNormal(DirectX::XMLoadFloat3( pPoints ), InverseR );

    for( UINT i = 1; i < Count; i++ )
    {
        DirectX::XMVECTOR Point = DirectX::XMVector3TransformNormal(DirectX::XMLoadFloat3( ( DirectX::XMFLOAT3* )( ( BYTE* )pPoints + i * Stride ) ),
                                                   InverseR );
                               ...
                               DirectX::XMVECTOR* pPlane3, DirectX::XMVECTOR* pPlane4, DirectX::XMVECTOR* pPlane5 )
{
    ...
    DirectX::XMVECTOR Origin = DirectX::XMLoadFloat3( &pVolume->Origin );
    DirectX::XMVECTOR Orientation = DirectX::XMLoadFloat4( &pVolume->Orientation );
    ...
    *pPlane0 = DirectX::XMPlaneNormalize( Plane0 );
    *pPlane1 = DirectX::XMPlaneNormalize( Plane1 );
    *pPlane2 = DirectX::XMPlaneNormalize( Plane2 );
    *pPlane3 = DirectX::XMPlaneNormalize( Plane3 );
    *pPlane4 = DirectX::XMPlaneNormalize( Plane4 );
    *pPlane5 = DirectX::XMPlaneNormalize( Plane5 );
}
...
VOID TransformSphere( Sphere* pOut, const Sphere* pIn, FLOAT Scale, FXMVECTOR Rotation, FXMVECTOR Translation )
{
    ////XMASSERT( pOut );
    ////XMASSERT( pIn );
    ////XMASSERT( DirectX::XMQuaternionIsUnit( Rotation ) );

    // Load the center of the sphere.
    DirectX::XMVECTOR Center = DirectX::XMLoadFloat3( &pIn->Center );
                              ...
                              FXMVECTOR Translation )
{
    ...
    DirectX::XMVECTOR Center = DirectX::XMLoadFloat3( &pIn->Center );
    DirectX::XMVECTOR Extents = DirectX::XMLoadFloat3( &pIn->Extents );
    ...
    // Store center and extents.
    DirectX::XMStoreFloat3( &pOut->Center, ( Min + Max ) * 0.5f );
    DirectX::XMStoreFloat3( &pOut->Extents, ( Max - Min ) * 0.5f );
                           ...
                           FXMVECTOR Translation )
{
    ...
    DirectX::XMVECTOR Center = DirectX::XMLoadFloat3( &pIn->Center );
    DirectX::XMVECTOR Extents = DirectX::XMLoadFloat3( &pIn->Extents );
    DirectX::XMVECTOR Orientation = DirectX::XMLoadFloat4( &pIn->Orientation );

    ////XMASSERT( DirectX::XMQuaternionIsUnit( Orientation ) );
    ...
    // Store the box.
    DirectX::XMStoreFloat3( &pOut->Center, Center );
    DirectX::XMStoreFloat3( &pOut->Extents, Extents );
    DirectX::XMStoreFloat4( &pOut->Orientation, Orientation );
...
VOID TransformFrustum( Frustum* pOut, const Frustum* pIn, FLOAT Scale, FXMVECTOR Rotation, FXMVECTOR Translation )
{
    ...
    // Load the frustum.
    DirectX::XMVECTOR Origin = DirectX::XMLoadFloat3( &pIn->Origin );
    DirectX::XMVECTOR Orientation = DirectX::XMLoadFloat4( &pIn->Orientation );
...
BOOL IntersectPointSphere( FXMVECTOR Point, const Sphere* pVolume )
{
    ...
    DirectX::XMVECTOR Center = DirectX::XMLoadFloat3( &pVolume->Center );
...
BOOL IntersectPointAxisAlignedBox( FXMVECTOR Point, const AxisAlignedBox* pVolume )
{
    ...
    DirectX::XMVECTOR Center = DirectX::XMLoadFloat3( &pVolume->Center );
    DirectX::XMVECTOR Extents = DirectX::XMLoadFloat3( &pVolume->Extents );
...
{
    static const DirectX::XMVECTORU32 SelectW = {XM_SELECT_0, DirectX::XM_SELECT_0, DirectX::XM_SELECT_0, DirectX::XM_SELECT_1};
    static const DirectX::XMVECTORU32 SelectZ = {XM_SELECT_0, DirectX::XM_SELECT_0, DirectX::XM_SELECT_1, DirectX::XM_SELECT_0};
    ...
    Planes[0] = DirectX::XMVectorSelect( BasePlanes[0], DirectX::XMVectorSplatX(  DirectX::XMLoadFloat( &pVolume->Near ) ),
                                SelectW );
    ...
    Planes[2] = DirectX::XMVectorSelect( BasePlanes[2], DirectX::XMVectorSplatX( -XMLoadFloat( &pVolume->RightSlope ) ),
                                SelectZ );
    Planes[3] = DirectX::XMVectorSelect( BasePlanes[3], DirectX::XMVectorSplatX(  DirectX::XMLoadFloat( &pVolume->LeftSlope ) ),
                                SelectZ );
    Planes[4] = DirectX::XMVectorSelect( BasePlanes[4], DirectX::XMVectorSplatX( -XMLoadFloat( &pVolume->TopSlope ) ),
                                SelectZ );
    Planes[5] = DirectX::XMVectorSelect( BasePlanes[5], DirectX::XMVectorSplatX(  DirectX::XMLoadFloat( &pVolume->BottomSlope ) ),
                                SelectZ );
...
// pp 21-28, 1997.
//-----------------------------------------------------------------------------//
BOOL IntersectRayTriangle(FXMVECTOR Origin, FXMVECTOR Direction, FXMVECTOR V0, CXMVECTOR V1, CXMVECTOR V2, FLOAT* pDist, FLOAT* uu, FLOAT* vv)
{
    ////XMASSERT( pDist );
    ...
    DirectX::XMVECTOR u2 = u * inv_det;
    DirectX::XMVECTOR v2 = v * inv_det;
    // u * inv_det and v * inv_det are the barycentric cooridinates of the intersection.

    DirectX::XMStoreFloat(uu, u2);
    DirectX::XMStoreFloat(vv, v2);

    // Store the x-component to *pDist
    DirectX::XMStoreFloat(pDist, t);
...
BOOL IntersectRayAxisAlignedBox( FXMVECTOR Origin, FXMVECTOR Direction, const AxisAlignedBox* pVolume, FLOAT* pDist )
{
    ...
    {
        // Store the x-component to *pDist
        DirectX::XMStoreFloat( pDist, t_min );
        return TRUE;
    }
...
BOOL IntersectRayOrientedBox( FXMVECTOR Origin, FXMVECTOR Direction, const OrientedBox* pVolume, FLOAT* pDist )
{
    ...
    static const DirectX::XMVECTORI32 SelectY =
    {
        DirectX::XM_SELECT_0, DirectX::XM_SELECT_1, DirectX::XM_SELECT_0, DirectX::XM_SELECT_0
    };
    static const DirectX::XMVECTORI32 SelectZ =
    {
...
BOOL IntersectTriangleTriangle( FXMVECTOR A0, FXMVECTOR A1, FXMVECTOR A2, CXMVECTOR B0, CXMVECTOR B1, CXMVECTOR B2 )
{
    ...
    if( DirectX::XMComparisonAllTrue( BDistIsLessCR ) || DirectX::XMComparisonAllTrue( BDistIsGreaterCR ) )
        return FALSE;
    ...
    // If all the points are on the same side we don't intersect.
    if( DirectX::XMComparisonAllTrue( ADistIsLessCR ) || DirectX::XMComparisonAllTrue( ADistIsGreaterCR ) )
        return FALSE;

    // Special case for co-planar triangles.
    if( DirectX::XMComparisonAllTrue( ADistIsZeroCR ) || DirectX::XMComparisonAllTrue( BDistIsZeroCR ) )
    {
    ...
    if( XMVector3AllTrue( DirectX::XMVectorSelect( ADistIsGreaterEqual, ADistIsLess, Select0111 ) ) ||
        XMVector3AllTrue( DirectX::XMVectorSelect( ADistIsGreater, ADistIsLessEqual, Select0111 ) ) )
    {
        // A0 is singular, crossing from positive to negative.
        AA0 = A0; AA1 = A1; AA2 = A2;
        bPositiveA = true;
    }
    else if( XMVector3AllTrue( DirectX::XMVectorSelect( ADistIsLessEqual, ADistIsGreater, Select0111 ) ) ||
             XMVector3AllTrue( DirectX::XMVectorSelect( ADistIsLess, ADistIsGreaterEqual, Select0111 ) ) )
    {
        // A0 is singular, crossing from negative to positive.
        AA0 = A0; AA1 = A2; AA2 = A1;
        bPositiveA = false;
    }
    else if( XMVector3AllTrue( DirectX::XMVectorSelect( ADistIsGreaterEqual, ADistIsLess, Select1011 ) ) ||
             XMVector3AllTrue( DirectX::XMVectorSelect( ADistIsGreater, ADistIsLessEqual, Select1011 ) ) )
    {
        // A1 is singular, crossing from positive to negative.
        AA0 = A1; AA1 = A2; AA2 = A0;
        bPositiveA = true;
    }
    else if( XMVector3AllTrue( DirectX::XMVectorSelect( ADistIsLessEqual, ADistIsGreater, Select1011 ) ) ||
             XMVector3AllTrue( DirectX::XMVectorSelect( ADistIsLess, ADistIsGreaterEqual, Select1011 ) ) )
    {
        // A1 is singular, crossing from negative to positive.
        AA0 = A1; AA1 = A0; AA2 = A2;
        bPositiveA = false;
    }
    else if( XMVector3AllTrue( DirectX::XMVectorSelect( ADistIsGreaterEqual, ADistIsLess, Select1101 ) ) ||
             XMVector3AllTrue( DirectX::XMVectorSelect( ADistIsGreater, ADistIsLessEqual, Select1101 ) ) )
    {
        // A2 is singular, crossing from positive to negative.
        AA0 = A2; AA1 = A0; AA2 = A1;
        bPositiveA = true;
    }
    else if( XMVector3AllTrue( DirectX::XMVectorSelect( ADistIsLessEqual, ADistIsGreater, Select1101 ) ) ||
             XMVector3AllTrue( DirectX::XMVectorSelect( ADistIsLess, ADistIsGreaterEqual, Select1101 ) ) )
    {
    ...
    if( XMVector3AllTrue( DirectX::XMVectorSelect( BDistIsGreaterEqual, BDistIsLess, Select0111 ) ) ||
        XMVector3AllTrue( DirectX::XMVectorSelect( BDistIsGreater, BDistIsLessEqual, Select0111 ) ) )
    {
        // B0 is singular, crossing from positive to negative.
        BB0 = B0; BB1 = B1; BB2 = B2;
        bPositiveB = true;
    }
    else if(XMVector3AllTrue( DirectX::XMVectorSelect( BDistIsLessEqual, BDistIsGreater, Select0111 ) ) ||
        XMVector3AllTrue( DirectX::XMVectorSelect( BDistIsLess, BDistIsGreaterEqual, Select0111 ) ) )
    {
        // B0 is singular, crossing from negative to positive.
        BB0 = B0; BB1 = B2; BB2 = B1;
        bPositiveB = false;
    }
    else if(XMVector3AllTrue( DirectX::XMVectorSelect( BDistIsGreaterEqual, BDistIsLess, Select1011 ) ) ||
        XMVector3AllTrue( DirectX::XMVectorSelect( BDistIsGreater, BDistIsLessEqual, Select1011 ) ) )
    {
        // B1 is singular, crossing from positive to negative.
        BB0 = B1; BB1 = B2; BB2 = B0;
        bPositiveB = true;
    }
    else if(XMVector3AllTrue( DirectX::XMVectorSelect( BDistIsLessEqual, BDistIsGreater, Select1011 ) ) ||
        XMVector3AllTrue( DirectX::XMVectorSelect( BDistIsLess, BDistIsGreaterEqual, Select1011 ) ) )
    {
        // B1 is singular, crossing from negative to positive.
        BB0 = B1; BB1 = B0; BB2 = B2;
        bPositiveB = false;
    }
    else if(XMVector3AllTrue( DirectX::XMVectorSelect( BDistIsGreaterEqual, BDistIsLess, Select1101 ) ) ||
        XMVector3AllTrue( DirectX::XMVectorSelect( BDistIsGreater, BDistIsLessEqual, Select1101 ) ) )
    {
        // B2 is singular, crossing from positive to negative.
        BB0 = B2; BB1 = B0; BB2 = B1;
        bPositiveB = true;
    }
    else if(XMVector3AllTrue( DirectX::XMVectorSelect( BDistIsLessEqual, BDistIsGreater, Select1101 ) ) ||
        XMVector3AllTrue( DirectX::XMVectorSelect( BDistIsLess, BDistIsGreaterEqual, Select1101 ) ) )
    {
...
BOOL IntersectTriangleAxisAlignedBox( FXMVECTOR V0, FXMVECTOR V1, FXMVECTOR V2, const AxisAlignedBox* pVolume )
{
    ...
    Axis = ReplaceXMVectorPermute( e0, -e0, Permute0W1Z0Y0X );
    ...
    Axis = ReplaceXMVectorPermute( e1, -e1, Permute0W1Z0Y0X );
    p0 = DirectX::XMVector3Dot( TV0, Axis );
    ...
    Axis = ReplaceXMVectorPermute( e2, -e2, Permute0W1Z0Y0X );
    p0 = DirectX::XMVector3Dot( TV0, Axis );
    p1 = DirectX::XMVector3Dot( TV1, Axis );
    ...
    Axis = ReplaceXMVectorPermute( e0, -e0, Permute0Z0W1X0Y );
    p0 = DirectX::XMVector3Dot( TV0, Axis );
    // p1 = DirectX::XMVector3Dot( V1, Axis ); // p1 = p0;
    p2 = DirectX::XMVector3Dot( TV2, Axis );
    Min = DirectX::XMVectorMin( p0, p2 );
    ...
    Axis = ReplaceXMVectorPermute( e1, -e1, Permute0Z0W1X0Y );
    p0 = DirectX::XMVector3Dot( TV0, Axis );
    p1 = DirectX::XMVector3Dot( TV1, Axis );
    // p2 = DirectX::XMVector3Dot( V2, Axis ); // p2 = p1;
    Min = DirectX::XMVectorMin( p0, p1 );
    ...
    Axis = ReplaceXMVectorPermute( e2, -e2, Permute0Z0W1X0Y );
    p0 = DirectX::XMVector3Dot( TV0, Axis );
    p1 = DirectX::XMVector3Dot( TV1, Axis );
    // p2 = DirectX::XMVector3Dot( V2, Axis ); // p2 = p0;
    Min = DirectX::XMVectorMin( p0, p1 );
    ...
    Axis = ReplaceXMVectorPermute( e0, -e0, Permute1Y0X0W0Z );
    p0 = DirectX::XMVector3Dot( TV0, Axis );
    // p1 = DirectX::XMVector3Dot( V1, Axis ); // p1 = p0;
    p2 = DirectX::XMVector3Dot( TV2, Axis );
    Min = DirectX::XMVectorMin( p0, p2 );
    ...
    Axis = ReplaceXMVectorPermute( e1, -e1, Permute1Y0X0W0Z );
    p0 = DirectX::XMVector3Dot( TV0, Axis );
    p1 = DirectX::XMVector3Dot( TV1, Axis );
    // p2 = DirectX::XMVector3Dot( V2, Axis ); // p2 = p1;
    Min = DirectX::XMVectorMin( p0, p1 );
    ...
    Axis = ReplaceXMVectorPermute( e2, -e2, Permute1Y0X0W0Z );
    p0 = DirectX::XMVector3Dot( TV0, Axis );
    p1 = DirectX::XMVector3Dot( TV1, Axis );
    // p2 = DirectX::XMVector3Dot( V2, Axis ); // p2 = p0;
    Min = DirectX::XMVectorMin( p0, p1 );
...
BOOL IntersectSphereSphere( const Sphere* pVolumeA, const Sphere* pVolumeB )
{
    ...
    // Load A.
    DirectX::XMVECTOR CenterA = DirectX::XMLoadFloat3( &pVolumeA->Center );
    DirectX::XMVECTOR RadiusA = DirectX::XMVectorReplicatePtr( &pVolumeA->Radius );

    // Load B.
    DirectX::XMVECTOR CenterB = DirectX::XMLoadFloat3( &pVolumeB->Center );
...
BOOL IntersectSphereAxisAlignedBox( const Sphere* pVolumeA, const AxisAlignedBox* pVolumeB )
{
    ...
    DirectX::XMVECTOR SphereCenter = DirectX::XMLoadFloat3( &pVolumeA->Center );
    DirectX::XMVECTOR SphereRadius = DirectX::XMVectorReplicatePtr( &pVolumeA->Radius );

    DirectX::XMVECTOR BoxCenter = DirectX::XMLoadFloat3( &pVolumeB->Center );
    DirectX::XMVECTOR BoxExtents = DirectX::XMLoadFloat3( &pVolumeB->Extents );
...
BOOL IntersectSphereOrientedBox( const Sphere* pVolumeA, const OrientedBox* pVolumeB )
{
    ...
    DirectX::XMVECTOR BoxCenter = DirectX::XMLoadFloat3( &pVolumeB->Center );
    DirectX::XMVECTOR BoxExtents = DirectX::XMLoadFloat3( &pVolumeB->Extents );
    DirectX::XMVECTOR BoxOrientation = DirectX::XMLoadFloat4( &pVolumeB->Orientation );

    ////XMASSERT( DirectX::XMQuaternionIsUnit( BoxOrientation ) );
...
BOOL IntersectAxisAlignedBoxAxisAlignedBox( const AxisAlignedBox* pVolumeA, const AxisAlignedBox* pVolumeB )
{
    ...
    DirectX::XMVECTOR CenterA = DirectX::XMLoadFloat3( &pVolumeA->Center );
    DirectX::XMVECTOR ExtentsA = DirectX::XMLoadFloat3( &pVolumeA->Extents );

    DirectX::XMVECTOR CenterB = DirectX::XMLoadFloat3( &pVolumeB->Center );
    DirectX::XMVECTOR ExtentsB = DirectX::XMLoadFloat3( &pVolumeB->Extents );
...
BOOL IntersectOrientedBoxOrientedBox( const OrientedBox* pVolumeA, const OrientedBox* pVolumeB )
{
    ...
    DirectX::XMVECTOR A_quat = DirectX::XMLoadFloat4( &pVolumeA->Orientation );
    DirectX::XMVECTOR B_quat = DirectX::XMLoadFloat4( &pVolumeB->Orientation );

    ////XMASSERT( DirectX::XMQuaternionIsUnit( A_quat ) );
    ////XMASSERT( DirectX::XMQuaternionIsUnit( B_quat ) );

    DirectX::XMVECTOR Q = DirectX::XMQuaternionMultiply( A_quat, DirectX::XMQuaternionConjugate( B_quat ) );
    DirectX::XMMATRIX R = DirectX::XMMatrixRotationQuaternion( Q );

    // Compute the translation of B relative to A.
    DirectX::XMVECTOR A_cent = DirectX::XMLoadFloat3( &pVolumeA->Center );
    DirectX::XMVECTOR B_cent = DirectX::XMLoadFloat3( &pVolumeB->Center );
    ...
    DirectX::XMVECTOR h_A = DirectX::XMLoadFloat3( &pVolumeA->Extents );
    DirectX::XMVECTOR h_B = DirectX::XMLoadFloat3( &pVolumeB->Extents );
    ...
    d = DirectX::XMVector3Dot( t, ReplaceXMVectorPermute( RX0, -RX0, Permute0W1Z0Y0X ) );
    d_A = DirectX::XMVector3Dot( h_A, ReplaceXMVectorPermute( ARX0, ARX0, PermuteWZYX ) );
    d_B = DirectX::XMVector3Dot( h_B, ReplaceXMVectorPermute( AR0X, AR0X, PermuteWZYX ) );
    NoIntersection = DirectX::XMVectorOrInt( NoIntersection, 
                                    DirectX::XMVectorGreater( DirectX::XMVectorAbs(d), DirectX::XMVectorAdd( d_A, d_B ) ) );
    ...
    d = DirectX::XMVector3Dot( t, ReplaceXMVectorPermute( RX1, -RX1, Permute0W1Z0Y0X ) );
    d_A = DirectX::XMVector3Dot( h_A, ReplaceXMVectorPermute( ARX1, ARX1, PermuteWZYX ) );
    d_B = DirectX::XMVector3Dot( h_B, ReplaceXMVectorPermute( AR0X, AR0X, PermuteZWXY ) );
    NoIntersection = DirectX::XMVectorOrInt( NoIntersection, 
                                    DirectX::XMVectorGreater( DirectX::XMVectorAbs(d), DirectX::XMVectorAdd( d_A, d_B ) ) );
    ...
    d = DirectX::XMVector3Dot( t, ReplaceXMVectorPermute( RX2, -RX2, Permute0W1Z0Y0X ) );
    d_A = DirectX::XMVector3Dot( h_A, ReplaceXMVectorPermute( ARX2, ARX2, PermuteWZYX ) );
    d_B = DirectX::XMVector3Dot( h_B, ReplaceXMVectorPermute( AR0X, AR0X, PermuteYXWZ ) );
    NoIntersection = DirectX::XMVectorOrInt( NoIntersection, 
                                    DirectX::XMVectorGreater( DirectX::XMVectorAbs(d), DirectX::XMVectorAdd( d_A, d_B ) ) );
    ...
    d = DirectX::XMVector3Dot( t, ReplaceXMVectorPermute( RX0, -RX0, Permute0Z0W1X0Y ) );
    d_A = DirectX::XMVector3Dot( h_A, ReplaceXMVectorPermute( ARX0, ARX0, PermuteZWXY ) );
    d_B = DirectX::XMVector3Dot( h_B, ReplaceXMVectorPermute( AR1X, AR1X, PermuteWZYX ) );
    NoIntersection = DirectX::XMVectorOrInt( NoIntersection, 
                                    DirectX::XMVectorGreater( DirectX::XMVectorAbs(d), DirectX::XMVectorAdd( d_A, d_B ) ) );
    ...
    d = DirectX::XMVector3Dot( t, ReplaceXMVectorPermute( RX1, -RX1, Permute0Z0W1X0Y ) );
    d_A = DirectX::XMVector3Dot( h_A, ReplaceXMVectorPermute( ARX1, ARX1, PermuteZWXY ) );
    d_B = DirectX::XMVector3Dot( h_B, ReplaceXMVectorPermute( AR1X, AR1X, PermuteZWXY ) );
    NoIntersection = DirectX::XMVectorOrInt( NoIntersection, 
                                    DirectX::XMVectorGreater( DirectX::XMVectorAbs(d), DirectX::XMVectorAdd( d_A, d_B ) ) );
    ...
    d = DirectX::XMVector3Dot( t, ReplaceXMVectorPermute( RX2, -RX2, Permute0Z0W1X0Y ) );
    d_A = DirectX::XMVector3Dot( h_A, ReplaceXMVectorPermute( ARX2, ARX2, PermuteZWXY ) );
    d_B = DirectX::XMVector3Dot( h_B, ReplaceXMVectorPermute( AR1X, AR1X, PermuteYXWZ ) );
    NoIntersection = DirectX::XMVectorOrInt( NoIntersection, 
                                    DirectX::XMVectorGreater( DirectX::XMVectorAbs(d), DirectX::XMVectorAdd( d_A, d_B ) ) );
    ...
    d = DirectX::XMVector3Dot( t, ReplaceXMVectorPermute( RX0, -RX0, Permute1Y0X0W0Z ) );
    d_A = DirectX::XMVector3Dot( h_A, ReplaceXMVectorPermute( ARX0, ARX0, PermuteYXWZ ) );
    d_B = DirectX::XMVector3Dot( h_B, ReplaceXMVectorPermute( AR2X, AR2X, PermuteWZYX ) );
    NoIntersection = DirectX::XMVectorOrInt( NoIntersection, 
                                    DirectX::XMVectorGreater( DirectX::XMVectorAbs(d), DirectX::XMVectorAdd( d_A, d_B ) ) );
    ...
    d = DirectX::XMVector3Dot( t, ReplaceXMVectorPermute( RX1, -RX1, Permute1Y0X0W0Z ) );
    d_A = DirectX::XMVector3Dot( h_A, ReplaceXMVectorPermute( ARX1, ARX1, PermuteYXWZ ) );
    d_B = DirectX::XMVector3Dot( h_B, ReplaceXMVectorPermute( AR2X, AR2X, PermuteZWXY ) );
    NoIntersection = DirectX::XMVectorOrInt( NoIntersection, 
                                    DirectX::XMVectorGreater( DirectX::XMVectorAbs(d), DirectX::XMVectorAdd( d_A, d_B ) ) );
    ...
    d = DirectX::XMVector3Dot( t, ReplaceXMVectorPermute( RX2, -RX2, Permute1Y0X0W0Z ) );
    d_A = DirectX::XMVector3Dot( h_A, ReplaceXMVectorPermute( ARX2, ARX2, PermuteYXWZ ) );
    d_B = DirectX::XMVector3Dot( h_B, ReplaceXMVectorPermute( AR2X, AR2X, PermuteYXWZ ) );
    NoIntersection = DirectX::XMVectorOrInt( NoIntersection, 
                                    DirectX::XMVectorGreater( DirectX::XMVectorAbs(d), DirectX::XMVectorAdd( d_A, d_B ) ) );
...
INT IntersectSphereFrustum( const Sphere* pVolumeA, const Frustum* pVolumeB )
{
    ...
    // Load origin and orientation of the frustum.
    DirectX::XMVECTOR Origin = DirectX::XMLoadFloat3( &pVolumeB->Origin );
    DirectX::XMVECTOR Orientation = DirectX::XMLoadFloat4( &pVolumeB->Orientation );

    //XMASSERT( DirectX::XMQuaternionIsUnit( Orientation ) );

    // Load the sphere.
    DirectX::XMVECTOR Center = DirectX::XMLoadFloat3( &pVolumeA->Center );
...
INT IntersectOrientedBoxFrustum( const OrientedBox* pVolumeA, const Frustum* pVolumeB )
{
    ...
    // Load origin and orientation of the frustum.
    DirectX::XMVECTOR Origin = DirectX::XMLoadFloat3( &pVolumeB->Origin );
    DirectX::XMVECTOR FrustumOrientation = DirectX::XMLoadFloat4( &pVolumeB->Orientation );

    //XMASSERT( DirectX::XMQuaternionIsUnit( FrustumOrientation ) );

    // Load the box.
    DirectX::XMVECTOR Center = DirectX::XMLoadFloat3( &pVolumeA->Center );
    DirectX::XMVECTOR Extents = DirectX::XMLoadFloat3( &pVolumeA->Extents );
    DirectX::XMVECTOR BoxOrientation = DirectX::XMLoadFloat4( &pVolumeA->Orientation );
    ...
    BoxOrientation = DirectX::XMQuaternionMultiply( BoxOrientation, DirectX::XMQuaternionConjugate( FrustumOrientation ) );
...
INT IntersectFrustumFrustum( const Frustum* pVolumeA, const Frustum* pVolumeB )
{
    ...
    DirectX::XMVECTOR OriginB = DirectX::XMLoadFloat3( &pVolumeB->Origin );
    DirectX::XMVECTOR OrientationB = DirectX::XMLoadFloat4( &pVolumeB->Orientation );

    //XMASSERT( DirectX::XMQuaternionIsUnit( OrientationB ) );
    ...
    DirectX::XMVECTOR OriginA = DirectX::XMLoadFloat3( &pVolumeA->Origin );
    DirectX::XMVECTOR OrientationA = DirectX::XMLoadFloat4( &pVolumeA->Orientation );

    //XMASSERT( DirectX::XMQuaternionIsUnit( OrientationA ) );
    ...
    OrientationA = DirectX::XMQuaternionMultiply( OrientationA, DirectX::XMQuaternionConjugate( OrientationB ) );
                                 ...
                                 CXMVECTOR Plane3, CXMVECTOR Plane4, CXMVECTOR Plane5 )
{
    ...
    DirectX::XMVECTOR Center = DirectX::XMLoadFloat3( &pVolume->Center );
    DirectX::XMVECTOR Extents = DirectX::XMLoadFloat3( &pVolume->Extents );
    DirectX::XMVECTOR BoxOrientation = DirectX::XMLoadFloat4( &pVolume->Orientation );
...
INT IntersectTrianglePlane( FXMVECTOR V0, FXMVECTOR V1, FXMVECTOR V2, CXMVECTOR Plane )
{
    ...
    //XMASSERT( DirectX::XMPlaneIsUnit( Plane ) );
