...
INT LoadObj(LPCTSTR sFileName, TObjMesh* pOutObjMesh)
{
	...
	while (!feof(pFile))
	{
		...
		{ 
			// Vertex
			DirectX::XMFLOAT3& v = obj.vertices[vc++];
		...
		{
			// Normal
			//sscanf( buffer+2, "%f %f %f", &obj.normals[nc].x, &obj.normals[nc].y, &obj.normals[nc].z );
			//sscanf(buffer + 2, "%f %f %f", &obj.normals[nc].z, &obj.normals[nc].x, &obj.normals[nc].y);
			sscanf(buffer + 2, "%f %f %f", &obj.normals[nc].x, &obj.normals[nc].y, &obj.normals[nc].z);
			//obj.normals[nc].x = -obj.normals[nc].x;
			//obj.normals[nc].z = -obj.normals[nc].z;
			nc++;
		}
