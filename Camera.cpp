...
void Camera::SetOrthoLens(float width, float height, float zn, float zf)
{
	...
	DirectX::XMMATRIX P = DirectX::XMMatrixOrthographicLH(width, height, mNearZ, mFarZ);
	DirectX::XMStoreFloat4x4(&mProj, P);
	UpdateViewMatrix();
}
...
DirectX::XMFLOAT3 Camera::GetStrafeUpPos(DirectX::XMFLOAT3 pos, float d)
{
	DirectX::XMVECTOR s = DirectX::XMVectorReplicate(d);
	DirectX::XMVECTOR r = DirectX::XMLoadFloat3(&mUp);
	DirectX::XMVECTOR p = DirectX::XMLoadFloat3(&pos);
	DirectX::XMStoreFloat3(&pos, DirectX::XMVectorMultiplyAdd(s, r, p));
	return pos;
}

DirectX::XMFLOAT3 Camera::GetStrafePos(DirectX::XMFLOAT3 pos, float d)
{
	// mPosition += d*mRight
	DirectX::XMVECTOR s = DirectX::XMVectorReplicate(d);
...
void Camera::Pitch(float angle)
{
	...
	DirectX::XMStoreFloat3(&mUp, DirectX::XMVector3TransformNormal(XMLoadFloat3(&mUp), R));
	DirectX::XMStoreFloat3(&mLook, DirectX::XMVector3TransformNormal(XMLoadFloat3(&mLook), R));
}
...
void Camera::RotateY(float angle)
{
	...
	DirectX::XMStoreFloat3(&mRight, DirectX::XMVector3TransformNormal(XMLoadFloat3(&mRight), R));
	DirectX::XMStoreFloat3(&mUp, DirectX::XMVector3TransformNormal(XMLoadFloat3(&mUp), R));
	DirectX::XMStoreFloat3(&mLook, DirectX::XMVector3TransformNormal(XMLoadFloat3(&mLook), R));
}
